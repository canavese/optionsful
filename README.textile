...
h3. Telnet is the geek way. 

    You would like better to use an HTTP client software. I use "HTTP Client":http://ditchnet.org/httpclient/ on Mac OS X.
...
** @base_path@
*** the path be appended to the host. Default is */api*. 
...
(link: true, host: auto)
<pre>
    Link: "<http://localhost:3000/api/notes>; type=text/html; rel=help"
</pre>
